generate_chunked_plots_expression <- function(seurat_object, genes, chunk_size = 20, aesthetical = NULL) {
  
  # Check if the genes are present in the Seurat object
  available_genes <- intersect(genes, rownames(seurat_object@assays$RNA$data))  # Note that the assay objects is v5.
