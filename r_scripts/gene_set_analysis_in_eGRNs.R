...
generate_gene_set_visualizations_w_extra_plot_w_cluster_option_network_focused <- function(pando_obj, all_gene_sets, 
  ...
  for (gene_set_name in all_gs_names) {  
    ...
    if (!is.null(high_exp_plot)) {
        ...
        selected_genes <- identify_high_expression_genes(seurat_object = pando_obj@data,  
    ...
    expression_plot <- generate_chunked_plots_expression(
...
if (!is.null(network_focused_on)) { 
    ...
    if (!is.null(plot_separate)) {  
     ...
     expression_plot_all_TF <- generate_chunked_plots_expression(
      seurat_object = seurat_object,
    ...
    ggsave(expression_plot_all_TF_file, plot = expression_plot_all_TF, width = 30, height = 24 + 24*var_height_2, dpi = 300)   
     ...
     expression_plot_all_Target <- generate_chunked_plots_expression(
      seurat_object = seurat_object,
    ...
    # use used genes to create a single plot.   
     ...
     expression_plot_all <- generate_chunked_plots_expression(
      seurat_object = seurat_object,
      genes = plotted_genes,
      aesthetical = aesthetical
    )
...
identify_high_expression_genes <- function(seurat_object, selected_genes, cell_type_of_interest, group_by) {
  ...
  available_genes <- intersect(selected_genes, rownames(seurat_object@assays$RNA$data))
  ...
  )$RNA  # Extract the RNA assay average
